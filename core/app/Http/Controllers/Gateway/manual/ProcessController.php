...
class ProcessController extends Controller
{
    public static function process($request, $gateway, $amount, $deposit)
    {
        ...
        if ($request->has('sponser_profit')) {
            $self_profit = $deposit->plan->return_interest - $request->sponser_profit;
            $deposit->self_profit = $self_profit;
            $deposit->sponser_profit = $request->sponser_profit;
        }
