...
class ManageGatewayController extends Controller
{
    ...
    public function depositAccept(Request $request)
    {
        ...
        // Add the deposited amount to appropriate wallet
        switch ($booking->wallet_type) {
            case (new CurrentWallet())->getTable(): // CurrentWallet
                $wallet = $booking->user->current_wallet;
                $payment = CurrentWalletPayment::where('user_id', $booking->user->id)->firstOrNew();
                break;
            case (new SavingWallet())->getTable():
                $wallet = $booking->user->saving_wallet;
                $payment = SavingWalletPayment::where('user_id', $booking->user->id)->firstOrNew();
                break;
            case (new SharingWallet())->getTable():
                $wallet = $booking->user->sharing_wallet;
                $payment = SharingWalletPayment::where('user_id', $booking->user->id)->firstOrNew();
                break;
            case (new BusinessPackWallet())->getTable():
                $wallet = $booking->user->business_pack_wallet;
                $payment = new BusinessPackPayment;
                $type = (new BusinessPackWallet())->getTable();
                break;
            case (new BusinessValueWallet())->getTable():
                $wallet = $booking->user->business_value_wallet;
                $payment = new BusinessValuePayment;
                $type = (new BusinessValueWallet())->getTable();
                break;
            default:
                return redirect()->back()->withError("Only wallet deposits are accepted");
                break;
        }

        $wallet->amount += $booking->amount;
        $payment->user_id = $booking->user->id;
        $payment->wallet_id = $wallet->id;
        $payment->next_payment_time = Carbon::now()->addHours(24);

        if ($booking->wallet_type == "business_pack_wallets") {
        ...
        if ($booking->wallet_type == "business_value_wallets") {

            if ($request->sponser_profit) {
                // Validate Sponser profit
                if ($request->return_interest < $request->sponser_profit) {
                    return redirect()->back()->withError('Sponser profit should be less than total profit');
                }
                $self_profit = $request->return_interest - $request->sponser_profit;
        ...
        $type = strtoupper(str_replace('_', ' ', $booking->wallet_type));
