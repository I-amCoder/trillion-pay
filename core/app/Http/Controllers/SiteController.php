...
class SiteController extends Controller
{
    ...
    public function investmentPlan(Request $request)
    {
        ...
        $plans = Plan::where('status', 1)->when($request->wallet, function ($query) use ($request) {
            if(in_array($request->wallet,['business_pack_wallet','business_value_wallet'])){
                $query->where('plan_wallet', $request->wallet."s");
            }else{
                $query;
            }
        })->get();
    ...
    public function allInvestmentPlan(Request $request)
    {
        $pageTitle = "Investment Plan";
        $plans = Plan::where('status', 1)->when($request->wallet, function ($query) use ($request) {
            $query->where('plan_wallet', $request->wallet);
        })->get();
    ...
    public function investLog(Request $request)
    {
        ...
        $business_pack_invests = BusinessPackPayment::when($request->pack_date, function ($item) use ($request) {
                $item->whereDate('created_at', $request->pack_date);
            })->where('user_id', auth()->id())->paginate();
        $business_value_invests = BusinessValuePayment::when($request->value_date, function ($item) use ($request) {
                $item->whereDate('created_at', $request->value_date);
            })->where('user_id', auth()->id())->paginate();

        return view($this->template . 'user.invest_log', compact('pageTitle', 'business_value_invests','business_pack_invests'));
    }
