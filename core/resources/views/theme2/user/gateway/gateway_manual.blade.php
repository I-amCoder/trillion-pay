                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <ul class="list-group">
                            ...
                            <li class="list-group-item  text-light d-flex justify-content-between">
                                <span>{{ __('Reciept Wallet') }}:</span>
                                <span
                                    class="text-uppercase">{{ rtrim(str_replace('_', ' ', $deposit->wallet_type), 's') }}</span>
                            </li>
                            @if ($deposit->plan_id)
                                <li class="list-group-item  text-light d-flex justify-content-between">
                                    <span>{{ __('Plan') }}:</span>
                                    <span class="text-uppercase">{{ $deposit->plan->plan_name }}</span>
                                </li>
                            @endif
                        </ul>
                    </div>
                                ...
                                <div class="form-group mb-2 col-md-12">
                                    @if (
                                        ...
                                        <h5>Total Profit: <span
                                                class="text-danger">{{ number_format($deposit->plan->return_interest, 3) }}%</span>
                                        </h5>
                                        <div class="form-group col-md-12">
                                            <label for="" class="mb-2 mt-2">{{ __('Sponser Profit') }}</label>
                                            <input placeholder="Your sponser will get." name="sponser_profit" type="number"
                                                step="any" required value="{{ old('sponser_profit') }}"
                                                class="form-control @error('sponser_profit') is-invalid @enderror">
                                            @error('sponser_profit')
